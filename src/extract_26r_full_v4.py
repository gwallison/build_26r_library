# -*- coding: utf-8 -*-
"""
extract_26r_full_v4.py
----------------------
Extracts Form 26R data from PDFs and generates a consolidated parquet file
and an interactive HTML table.

Outputs:
    data/output/all_harvested_form26r_v4.parquet
    data/output/Form_26R_files_v4.html

Changes from extract_26r_full_v3.py:
- Bug fix: SECTION B header regex required a single literal space
  ("SECTION B." + one space + "WASTE DESCRIPTION"), but a large share of
  real forms render it with two spaces there. This silently
  dropped waste_code/waste_description/amount/unit for ~43% of rows.
  Fixed to accept one-or-more whitespace characters.
- Bug fix: contact phone regex only matched hyphen-separated numbers
  (724-549-8287). Broadened to also accept parenthesized area codes and
  dot separators ((724) 549-8287 / 412.249.3850), which together account
  for the large majority of previously-missed contact_phone values.
- New fields: municipality, county, state -- the waste-generation-location
  Municipality/County/State line that appears in Section A immediately
  before SECTION B. Extraction is scoped to the text window between the
  "describe location of waste generation and storage" answer and
  SECTION B, and requires a literal 'State' label, which disambiguates it
  from the earlier, unrelated Company Contact Municipality/County pair
  (which has no State field). Values are allowed to be blank or wrap
  onto a second line, but capture length is bounded to avoid ever
  matching across into a different labeled block if 'State' is absent
  nearby (some minority-template filings use a different field order and
  will simply be left null, same as other best-effort fields below).
- Output filenames and intermediate per-set JSON filenames versioned as
  _v4 so this run does not overwrite v3's saved intermediates/outputs.
"""
...
def parse_sections_a_and_b(page, current_form):
    ...
    # --- Municipality / County / State (waste generation location) ---
    # A second, unrelated Municipality/County pair appears earlier under the
    # Company Contact block (no State field, not near SECTION B). We avoid
    # that one by (a) only searching the window between the waste-location
    # answer and SECTION B, and (b) requiring a literal 'State' label.
    # Capture lengths are bounded so a missing 'State' nearby (a minority
    # template with a different field order) fails safely to None rather
    # than swallowing unrelated text further down the page.
    sec_b_boundary = re.search(r'SECTION B\.\s+WASTE DESCRIPTION', text_content, re.I)
    region_start = loc_match.end() if loc_match else 0
    region_end = sec_b_boundary.start() if sec_b_boundary else min(len(text_content), region_start + 600)
    muni_region = text_content[region_start:region_end]

    muni_match = re.search(
        r'Municipality\s*[\r\n]*[\s_]*([\s\S]{0,60}?)\s*County\s*[\r\n]*[\s_]*([\s\S]{0,50}?)\s*State\s*[\r\n]*[\s_]*([\s\S]{0,10}?)[\r\n]',
        muni_region, re.I
    )
    def _clean_label_value(raw):
        val = re.sub(r'[\r\n]+', ' ', raw).strip()
        # Drop stray OCR/mojibake junk (e.g. a garbled underscore separator)
        # that occasionally precedes the real value.
        return re.sub(r'^[^A-Za-z0-9]+', '', val).strip()

    if muni_match:
        muni_val = _clean_label_value(muni_match.group(1))
        county_val = _clean_label_value(muni_match.group(2))
        state_val = _clean_label_value(muni_match.group(3))
        if muni_val:
            current_form["municipality"] = muni_val
        if county_val:
            current_form["county"] = county_val
        if state_val:
            current_form["state"] = state_val
    ...
    if email_phone_match:
        ...
        # Accepts 724-549-8287, (724) 549-8287, and 412.249.3850 variants.
        phone_m = re.search(r'(\(?\d{3}\)?[\s.\-]*\d{3}[\s.\-]*\d{4})', content)
        if phone_m:
            current_form["contact_phone"] = phone_m.group(1).strip()
    ...
    # Fixed: header regex previously required exactly one space after
    # 'SECTION B.'; real forms often render two spaces there.
    sec_b_match = re.search(r'SECTION B\.\s+WASTE DESCRIPTION[\s\S]*', text_content, re.I)
    if sec_b_match:
...
def run_test_fixture(input_dir, output_dir, setname):
    ...
    output_file = os.path.join(output_dir, f"form26r_extraction_results__{setname}_v4.json")
...
def all_json_to_concat(dirset, rootdir, resdir, out_dir=None):
    ...
    for dset in dirset:
        ...
        jfn = os.path.join(resdir, f'form26r_extraction_results__{dset}_v4.json')
    ...
    if alldf:
        ...
        parq_path = os.path.join(out_dir, 'all_harvested_form26r_v4.parquet')
...
def make_form26r_html(parquet_path, output_dir):
    ...
    html = itables.to_html_datatable(
        out[['pdf_link', 'page_number', 'company_name',
             ...
             'waste_location', 'municipality', 'county', 'state', 'waste_code',
...
"""
    ...
    os.makedirs(output_dir, exist_ok=True)
    out_path = os.path.join(output_dir, 'Form_26R_files_v4.html')
