# -*- coding: utf-8 -*-
"""
extract_26r_full_v3.py
----------------------
Extracts Form 26R data from PDFs and generates a consolidated parquet file
and an interactive HTML table.

Outputs:
    data/output/all_harvested_form26r_v3.parquet
    data/output/Form_26R_files_v3.html

Changes from legacy extract_26r_full.py:
- Date regex: fuzzier label match to handle OCR variants (spurious digits,
  typos in 'Prepared', slash read as 'i'); expanded date pattern to handle
  hyphen separators and spelled-out months (e.g. '02 February 2017').
- Output filenames versioned as _v3.
"""
...
def all_json_to_concat(dirset, rootdir, resdir, out_dir=None):
    ...
    if alldf:
        ...
        parq_path = os.path.join(out_dir, 'all_harvested_form26r_v3.parquet')
...
"""
    ...
    os.makedirs(output_dir, exist_ok=True)
    out_path = os.path.join(output_dir, 'Form_26R_files_v3.html')
