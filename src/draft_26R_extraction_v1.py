# -*- coding: utf-8 -*-
"""
Created on Wed Mar 25 11:05:05 2026
...
# --- Output directory (tmp/ subdirectory of this file's location) ---
...
def run_test_fixture(input_dir, output_dir, setname):
    ...
    all_results = []
    print(f"Starting processing of {len(pdf_files)} PDFs...")
...
def all_json_to_concat(dirset, rootdir, resdir, out_dir=None):
    ...
    if alldf:
        ...
        final.to_parquet(os.path.join(out_dir, 'all_harvested_form26r_v2.parquet'))


# ---------------------------------------------------------------------------
# HTML table generation  (from "Make table of Form 26Rs" notebook section)
# ---------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    results = r"D:\PA_Form26r_PDFs\results"

    harvest_all_dirs(root_dir=root, res_dir=results, dir_set=dirset)
    all_json_to_concat(dirset, rootdir=root, resdir=results, out_dir=TMP_DIR)

    make_form26r_html(
        parquet_path=os.path.join(TMP_DIR, 'all_harvested_form26r_v2.parquet'),
        output_dir=TMP_DIR
    )
