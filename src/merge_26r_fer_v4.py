...
merge_26r_fer_v4.py
--------------------
Enriches the v4 Form 26R harvest with facility location data from linking_v3.
Same join logic as merge_26r_fer.py, pointed at the v4 base extraction
instead of v3. forms_26r.parquet / clusters_26r_fer.parquet are frozen
linking_v3 artifacts (built from an earlier extraction snapshot) -- v4 has
the identical (filename, page_number) key set as v3 and identical
company_name/waste_location values (the fields the pad-name clustering was
keyed on), so this re-join is expected to reproduce the same cluster_id /
anchor_pad_id assignments as v3_fer, just carrying v4's better-filled
waste_code/amount/contact_phone and new municipality/county/state fields.

Join chain (all left joins, no row explosion):
  1. all_harvested_form26r_v4.parquet
...
Outputs:
    data/output/all_harvested_form26r_v4_fer.parquet
    data/output/Form_26R_files_v4_fer.html
"""
...
V4_PARQUET   = os.path.join(OUTPUT_DIR, 'all_harvested_form26r_v4.parquet')
OUT_PARQUET  = os.path.join(OUTPUT_DIR, 'all_harvested_form26r_v4_fer.parquet')
OUT_HTML     = os.path.join(OUTPUT_DIR, 'Form_26R_files_v4_fer.html')
...
def build_merged(v4_path, forms_path, clusters_fer_path, fer_registry_path):
    print("Loading v4 harvest...")
    ...
    v4 = v4.merge(forms, on=['filename', 'page_number'], how='left')
    missing = v4['cluster_id'].isna().sum()
    if missing:
    ...
    v4 = v4.merge(clusters_fer, on='cluster_id', how='left')
    anchored = v4['anchor_pad_id'].notna().sum()
    print(f"  Anchored: {anchored:,} / {len(v4):,} rows ({100*anchored/len(v4):.1f}%)")
    ...
    fer_reg = fer_reg.rename(columns={'ADDRESS1': 'pad_address',
                                      'CITY':     'pad_city',
                                      'ZIP':      'pad_zip'})
    v4 = v4.merge(fer_reg, left_on='anchor_pad_id', right_on='WELL_PAD_ID', how='left')
    v4 = v4.drop(columns=['WELL_PAD_ID'])  # redundant with anchor_pad_id

    with_latlon = v4['WELL_PAD_LATITUDE'].notna().sum()
    print(f"  With lat/lon: {with_latlon:,} / {len(v4):,} rows ({100*with_latlon/len(v4):.1f}%)")

    # Confidence tier summary
    print("\n  Confidence tier breakdown:")
    tiers = [
        ('high   (>=0.87)', v4['anchor_confidence'] >= 0.87),
        ('medium (0.77)',    v4['anchor_confidence'] == 0.77),
        ('low    (0.60-0.65)', v4['anchor_confidence'].between(0.60, 0.65)),
        ('conflict (0.57)', v4['anchor_confidence'] == 0.57),
        ('none   (0.00)',   v4['anchor_confidence'] == 0.00),
    ]
    for label, mask in tiers:
        n = int(mask.sum())
        ll = int((mask & v4['WELL_PAD_LATITUDE'].notna()).sum())
...
def make_html(df, out_path):
    ...
    display_cols = [
        ...
        'company_name', 'waste_location', 'municipality', 'county', 'state', 'date_prepared',
    ...
    <h2>All files with Form 26R — FER Enriched (v4)</h2>
